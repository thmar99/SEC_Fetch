...
'''with open(f'master_reports_{ticker}.json','w')as f:
...
def StructFinancials(report,headers):
    ...
    report_soup = BeautifulSoup(content,'lxml')#html
...
def CleanSheet(DataFrame):   
    ...
    df_cleaned.to_csv('CleanedSheet.csv')

    return df_cleaned

def CreateDataframe(financials,csv_nam):
    ...
    if len(income_df.columns) != len(income_header):
        ...
        income_df.to_csv('df_raws_c.csv')
...
def EdgarFetcher():
    ...
    #User input
    dateRange = input('Insert Date range (Format YYYYMMDD): ')
    ticker = input('Insert Company ticker: ')
    cik = getCIK(ticker)
    formType = input('Insert form type(10-k,10-q,8-k): ')
